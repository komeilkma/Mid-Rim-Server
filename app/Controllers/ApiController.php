...
class ApiController extends ResourceController
{
    ...
    public function Get_Gateway_Data($id)
    {
        ...
        $db->connect();
        $query = $db->table('devices')->where('DeviceID', $id)->get();
        $gw_data = $query->getRow();
        if ($query->getNumRows() === 1) {
        $gateway_data = [
            'id'       => $gw_data->id,
            'DeviceID'       => $gw_data->DeviceID,
            ...
            'Object'        => $gw_data->Object,
            'Config'        => $gw_data->Config,
        ];
        ...
        return $this->failNotFound("Device Was Not Found");
